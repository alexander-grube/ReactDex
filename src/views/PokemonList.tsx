...
const Example = () => {
  ...
  const { data, isError, isFetching, isLoading, refetch } =
    useQuery<PokemonListState>({
      ...
      queryFn: async () => {
        const fetchURL = new URL('/api/v2/pokemon/', 'https://pokeapi.co');
        ...
        for (const pokemon of json.results) {
          ...
          pokemon.image = `https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/other/official-artwork/${
            pokemonURL.pathname.split('/')[4]
          }.png`;
          pokemon.abilities = [
            { id: 1, name: 'Ability 1' },
            { id: 2, name: 'Ability 2' },
    ...
    () => [
      {
        ...
        Cell: ({ cell }) => {
          return (
            <Chip
              color="primary"
              label={cell.getValue<string>().toLocaleUpperCase()}
            />
          );
        },
      },
